...
async function api(req,res,url){
 ...
 if(req.method==='GET'&&p.startsWith('/api/lots/')&&p.endsWith('/messages')){
  ...
  const allowed=isCollector||(u.role==='buyer'&&myQuote);
  if(!allowed)return json(req,res,403,{error:'You are not a participant in this lot'});
  const messages=(lot.messages||[]).filter(m=>!quoteId||m.quote_id===quoteId||(!m.quote_id&&quoteId===null));
