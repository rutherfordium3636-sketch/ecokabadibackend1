...
async function api(req,res,url){
 ...
 if(req.method==='GET'&&p.startsWith('/api/lots/')&&p.endsWith('/messages')){
  ...
  if(isBuyer&&quoteId!==myQuote.id)return json(req,res,403,{error:'You can only access your own quote conversation'});
  const messages=(lot.messages||[]).filter(m=>isCollector ? (!quoteId||m.quote_id===quoteId||(!m.quote_id&&quoteId===null)) : m.quote_id===myQuote.id);
 ...
 if(req.method==='POST'&&p.startsWith('/api/lots/')&&p.endsWith('/messages')){
  ...
  if(!lot)return json(req,res,404,{error:'Lot not found'});
  normalizeQuotes(lot);const x=await body(req);const msg=String(x.message||'').trim();const quoteId=String(x.quote_id||'').trim()||null;
  if(!msg||msg.length>1000)return json(req,res,400,{error:'Message must be 1–1000 characters'});
  const q=quoteId?lot.quotes.find(q=>q.id===quoteId):null;
  const isCollector=u.role==='collector'&&lot.user_id===u.id;const isBuyer=u.role==='buyer'&&q&&q.buyer_id===u.id;
